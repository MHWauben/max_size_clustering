...
while(dat_size > 0){
  ...
  repeat{
    ...
    thirdlargestsize <- ifelse(is.na(nth(clustersums$size, 3)), 0, 30)
    accurate_clusters <- (clustersizes %>% group_by(labels) %>% summarise(size = n()) %>% filter(size >= thirdlargestsize) %>% select(labels))[[1]]
    if(maxsize <= 59){
      break
    }
    i = i + 1
